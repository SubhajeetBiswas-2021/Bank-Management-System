...
public class SignupOne extends JFrame implements ActionListener{
    ...
    SignupOne(){
        ...
        random = Math.abs((ran.nextLong() % 9000L) + 1000L);  //Math.abs gives the positive value

        JLabel formno = new JLabel("APPLICATION FORM NO." + random );
        formno.setFont(new Font ("Raleway",Font.BOLD,38));
        formno.setBounds(140,20,600,40); //setBound will only work when the setlayout is null
        add(formno);

        JLabel personDetails = new JLabel("Page1:Personal Details");
        personDetails.setFont(new Font ("Raleway",Font.BOLD,22));
        personDetails.setBounds(290,80,400,40); //setBound will only work when the setlayout is null
        add(personDetails);
        ...
        ButtonGroup gendergroup = new ButtonGroup();    //by this any one of the radiobutton will get selected
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        String formno = "" + random;     //form is a long no so by using ("" + ) converted the long number into string and here + means concat
        String name = nameTextField.getText(); 
        String fname = fnameTextField.getText();
        String dob = ((JTextField)dateChooser.getDateEditor().getUiComponent()).getText(); //getUiComponent returns textfeild so concatted with JtextFeild
        String gender = null;
        if(male.isSelected()){
            gender = "Male";
        ...
        try{
            if(name.equals("")){
                JOptionPane.showMessageDialog(null,"Name is Required");   //it shows an pop up using JOptionPane
            }else {
                Conn c = new Conn();            //making object of conn class here for establishing connection
                String query = "insert into signup values('"+formno+"','"+name+"','"+fname+"','"+dob+"','"+gender+"','"+email+"','"+marital+"','"+address+"','"+city+"','"+pin+"','"+state+"')";      //DML 
